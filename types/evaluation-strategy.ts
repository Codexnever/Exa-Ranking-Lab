...
export const STRATEGY_BENCHMARK_POLICY_VERSION =
  "1" as const
...
export type StrategyExecutionSource =
  | "native"
  | "imported"

export type StrategyLatencyType =
  | "end_to_end"
  | "retrieval_only"
  | "rerank_only"
  | "custom"
...
export interface StrategyExecutionDocument {
  documentKey: string
  canonicalUrl: string
  rawUrl: string
...
export interface StrategyLatencySummary {
  available: boolean
...
export interface StrategyErrorSummary {
  hardNegativeCandidateCount: number
  highCriticalCount: number

  top5Grade0Count: number
  outranksGrade2Count: number

  queriesWithHighCritical: number

  candidateRatePerQuery: number
  highCriticalQueryRate: number
}

export interface StrategyStageSummary {
  available: boolean
...
export interface StrategyQueryResult {
  ...
  highCriticalHardNegativeCount: number
...
export interface StrategyWinLoss {
  evaluationQueryId: string
  queryText: string
...
export interface StrategyBenchmark {
  benchmarkPolicyVersion:
    typeof STRATEGY_BENCHMARK_POLICY_VERSION
