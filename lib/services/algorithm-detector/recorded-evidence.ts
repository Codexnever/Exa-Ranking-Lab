...
export function sanitizeRecordedEvidence(value: unknown): RecordedDetectorEvidence {
  ...
  for (const [key, allowed] of [
    ...
    if (result.text[key] && !(allowed as readonly string[]).includes(result.text[key])) {
      delete result.text[key]
    }
  }

  for (const key of ["historicalBaselineAvailable", "confidenceCapped"]) {
    if (typeof flags[key] === "boolean") {
      result.flags[key] = flags[key]
    }
  }

  if (Array.isArray(source.gates)) {
    result.gates = source.gates.slice(0, 12).flatMap((value) => {
      const gate = object(value)
      if (typeof gate.code !== "string" || typeof gate.message !== "string") return []
      return [
        {
          code: gate.code.slice(0, 64),
          message: gate.message.slice(0, 1000),
          ...(typeof gate.passed === "boolean" ? { passed: gate.passed } : {}),
        },
      ]
    })
  }

  if (result.text.baselineAvailabilityReasonCode === "provider_failure") {
    result.text.baselineAvailabilityReason = "Historical baseline provider was unavailable."
  }
...
export function readRecordedEvidence(document: Record<string, unknown>): RecordedDetectorEvidence {
  ...
  const evidence = parsed(document.evidenceJson)
  const thresholds = parsed(document.thresholdsJson)
  const confidence = parsed(document.confidenceJson)

  const source = {
    ...thresholds,
    ...evidence,
    ...Object.fromEntries(Object.entries(document).filter(([, value]) => value !== null && value !== undefined)),
    confidencePercentage: confidence.percentage ?? document.confidencePercentage,
    confidenceCapped: confidence.confidenceCapped,
  ...
  return sanitizeRecordedEvidence({
    ...
    text: source,
    flags: source,
    gates: evidence.detectionReasons,
  })
}