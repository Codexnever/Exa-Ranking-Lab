  ...
  for (const schema of schemas) {
    ...
    for (const index of schema.indexes) {
      ...
      if (!existing) {
        ...
        if (!dryRun) { await db.createIndex(databaseId, schema.id, index.key, index.type, index.attributes); await waitIndex(schema.id, index.key) }
      } else if (existing.type !== index.type || JSON.stringify(existing.attributes) !== JSON.stringify(index.attributes)) {
