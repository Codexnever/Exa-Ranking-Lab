...
export interface ParsedJudgmentLabel { resultUrl: string; grade: 0 | 1 | 2; rationale?: string }
export function parseJudgmentBatch(value: unknown): { snapshotId: string; labels: ParsedJudgmentLabel[] } {
  const body = record(value)
  if (Object.keys(body).some(key => !["snapshotId", "labels"].includes(key))) throw invalid("Judgment request contains unsupported authoritative fields")
  assertRouteId("snapshotId", body.snapshotId)
  if (!Array.isArray(body.labels) || body.labels.length < 1 || body.labels.length > MAX_JUDGMENT_BATCH) throw invalid(`labels must contain 1-${MAX_JUDGMENT_BATCH} entries`)
  const labels = body.labels.map((item): ParsedJudgmentLabel => {
    const label = record(item)
    if (Object.keys(label).some(key => !["resultUrl", "grade", "rationale"].includes(key))) throw invalid("Label contains unsupported authoritative fields")
    if (typeof label.resultUrl !== "string" || !label.resultUrl.trim() || label.resultUrl.length > MAX_RESULT_URL) throw invalid(`resultUrl must be 1-${MAX_RESULT_URL} characters`)
    if (label.grade !== 0 && label.grade !== 1 && label.grade !== 2) throw invalid("grade must be exactly 0, 1, or 2")
    if (label.rationale !== undefined && (typeof label.rationale !== "string" || label.rationale.length > MAX_RATIONALE)) throw invalid(`rationale must be at most ${MAX_RATIONALE} characters`)
    return { resultUrl:label.resultUrl.trim(),grade:label.grade,...(label.rationale?.trim()?{rationale:label.rationale.trim()}:{}) }
  })
  return { snapshotId:body.snapshotId,labels }
}
export function parseAdjudicationInput(value: unknown): { grade: 0 | 1 | 2; rationale: string } {
  const body = record(value)
  if (Object.keys(body).some(key => !["grade", "rationale"].includes(key))) throw invalid("Adjudication request contains unsupported fields")
  if (body.grade !== 0 && body.grade !== 1 && body.grade !== 2) throw invalid("grade must be exactly 0, 1, or 2")
  if (typeof body.rationale !== "string" || !body.rationale.trim() || body.rationale.length > MAX_RATIONALE) throw invalid(`rationale must be 1-${MAX_RATIONALE} characters`)
  return { grade:body.grade,rationale:body.rationale.trim() }
}
