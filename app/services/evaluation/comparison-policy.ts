...
export function direction(delta: number | null): QualityChangeDirection {
  if (delta === null) return "unavailable";
  if (Math.abs(delta) < QUALITY_COMPARISON_POLICY.stableEpsilon) return "stable";
  return delta > 0 ? "improved" : "degraded";
}

export function magnitude(delta: number | null): QualityChangeMagnitude {
  if (delta === null) return "unavailable";
  const value = Math.abs(delta);
  if (value < QUALITY_COMPARISON_POLICY.stableEpsilon) return "stable";
  if (value < QUALITY_COMPARISON_POLICY.smallUpperBound) return "small";
  if (value < QUALITY_COMPARISON_POLICY.moderateUpperBound) return "moderate";
  return "large";
}

export function metricDelta(before: number[], after: number[]): MetricDelta {
  if (before.length !== after.length) {
    throw new TypeError("Metric cohorts must be aligned");
  }
...
export function interpretation(
  ...
  drift?: { mean: number; substantial: boolean }
): QualityAwareInterpretation {
  ...
  const recallStable = recall?.direction === "stable" || recall?.magnitude === "small";
  ...
  if (drift) {
    text += drift.substantial
      ? d === "stable"
        ? " Ranking changed substantially, but measured benchmark relevance remained stable."
        : ` Substantial ranking change coincided with ${d === "improved" ? "improved" : "lower"} measured relevance.`
      : d === "degraded"
