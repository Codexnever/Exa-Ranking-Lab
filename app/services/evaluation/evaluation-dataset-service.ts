...
export interface EvaluationRepository {
  ...
  updateDataset(id: string, data: Partial<Pick<EvaluationDatasetVersion, "queryCount" | "judgmentCount" | "conflictCount" | "updatedAt">>): Promise<EvaluationDatasetVersion>
...
export class AppwriteEvaluationRepository implements EvaluationRepository {
  ...
  async updateDataset(id: string, data: Partial<Pick<EvaluationDatasetVersion, "queryCount" | "judgmentCount" | "conflictCount" | "updatedAt">>): Promise<EvaluationDatasetVersion> {
    try {
...
export class EvaluationDatasetService {
  ...
  freezeReadiness(dataset: EvaluationDatasetVersion, queries: EvaluationQuery[], judgments: RelevanceJudgment[] = []): QueryFoundationReadiness {
    ...
    const judgmentFoundationReady=queryFoundationReady&&checks.noConflicts&&checks.judgmentCountConsistent&&checks.conflictCountConsistent&&checks.noOrphanJudgments&&checks.hasAcceptedJudgments&&checks.everyQueryHasAcceptedJudgment
    return {queryFoundationReady,judgmentFoundationReady,fullEvaluationFreezeReady:false,checks,pendingPhases:[...(judgmentFoundationReady?[]:["judgments" as const]),...(checks.noConflicts&&checks.conflictCountConsistent?[]:["conflict_resolution" as const]),"final_freeze"]}
  }
