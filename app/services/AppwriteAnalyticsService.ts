// app/services/AppwriteAnalyticsService.ts
  ...
  import type { AnalyticsData, QueryConfig, RankingSnapshot, EnhancedAnalyticsData, ExecutionFrequency, DataFreshness } from '@/lib/type';
  export class AppwriteAnalyticsService extends AnalyticsService {
    ...
    try {
      // Use parent's getAnalytics - it already returns EnhancedAnalyticsData
      const baseAnalytics = await super.getAnalytics(userId, timeRangeMs);
      
      // Get the snapshots for enhanced calculations
      ...
      // Add Appwrite-specific enhancements
      const appwriteMetrics = this.calculateAppwriteSpecificMetrics(snapshots);
      const enhancedMetrics = this.calculateEnhancedPerformanceMetrics(snapshots);

      // Use shared analytics logic if available
      const timeRange = timeRangeMs ? this.getTimeRangeString(timeRangeMs) : '30d';
      const sharedAnalytics = analyticsCalculations(queries, snapshots, timeRange);
  ...
  try {
     ...
     */
     protected calculateEnhancedPerformanceMetrics(snapshots: RankingSnapshot[]) {
    const responseTimes = snapshots
      .map(s => s.metadata?.responseTime)
      .filter((time): time is number => typeof time === 'number');

    // FIXED: Pass snapshots instead of executionTimes
  ...
  // dataFreshnessScore: dataFreshness.freshnessScore, // Remove invalid property
  executionEfficiency: executionFrequency.efficiency,
     ...
     */
    protected calculateRankingMetrics(snapshotsByQuery: Record<string, RankingSnapshot[]>): { stabilityScore: number; volatilityIndex: number } {
  ...
  protected calculateExecutionFrequency(snapshots: RankingSnapshot[]): ExecutionFrequency {
    ...
    // Extract execution times from snapshots
