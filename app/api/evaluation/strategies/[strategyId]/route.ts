...
async function handler(_r:NextRequest,c:SecurityContext,route:{params:Promise<{strategyId:string}>})
{
    try{
        const{strategyId}=await route.params;assertRouteId("strategy ID",strategyId);
        return NextResponse.json(await evaluationStrategyService.getStrategy(c.user.$id,strategyId))}
        catch(e){
            return NextResponse.json({error:e instanceof Error?e.message:"Strategy read failed"},
