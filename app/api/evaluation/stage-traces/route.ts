...
async function handler(
  ...
  try {
    if (request.method === "POST") {
      ...
      // Reject oversized requests early when the client provides content length.
      if (
        Number.isFinite(declaredLength) &&
        declaredLength > EVALUATION_STAGE_TRACE_POLICY.maxRequestBytes
      ) {
        throw new EvaluationError(
          "PROVENANCE_LIMIT",
          "Stage trace request exceeds the body-size limit",
          413,
        )
      }
      ...
      // Verify the actual payload size because content-length cannot be trusted alone.
      if (
        Buffer.byteLength(raw, "utf8") >
        EVALUATION_STAGE_TRACE_POLICY.maxRequestBytes
      ) {
        throw new EvaluationError(
          "PROVENANCE_LIMIT",
          "Stage trace request exceeds the body-size limit",
      ...
      const trace = await evaluationStageTraceService.create(
        context.user.$id,
        input,
      )
    ...
    const traces = await evaluationStageTraceService.list(
      context.user.$id,
      {
        ...
        datasetVersionId:
          params.get("datasetVersionId") ?? undefined,
    ...
    // Preserve expected validation and evaluation errors for the API client.
    if (
    ...
    return NextResponse.json(
      { error: "Stage trace operation failed" },
      { status: 500 },
    )
  }
}
