...
async function handler(_request:NextRequest,context:SecurityContext,route:{params:Promise<{traceId:string}>}){try{const{traceId}=await route.params;assertRouteId("trace ID",traceId);return NextResponse.json(await evaluationStageTraceService.get(context.user.$id,traceId))}catch(error){if(error instanceof EvaluationError||error instanceof TypeError)return NextResponse.json({error:error.message},{status:error instanceof EvaluationError?error.status:409});return NextResponse.json({error:"Failed to read stage trace"},{status:500})}}
