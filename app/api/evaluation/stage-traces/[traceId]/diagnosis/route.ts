...
async function handler(_request:NextRequest,context:SecurityContext,
    route:{params:Promise<{traceId:string}>
})
{
    try{const{traceId}=await route.params;
    assertRouteId("trace ID",traceId);
    return NextResponse.json(
        await evaluationStageDiagnosisService.diagnose(context.user.$id,traceId))}
        catch(error){if(error instanceof EvaluationError||error instanceof TypeError)
            return NextResponse.json({error:error.message,code:error instanceof EvaluationError?error.code:"INVALID_STATE"},
                {status:error instanceof EvaluationError?error.status:409});
                console.error("[StageDiagnosis] failed",error);
                return NextResponse.json({error:"Failed to diagnose stage trace"},{status:500})}}
