...
async function handler(_request:NextRequest,context:SecurityContext,route:{params:Promise<{id:string;runId:string}>}){try{const{id,runId}=await route.params;assertRouteId("dataset ID",id);assertRouteId("run ID",runId);return NextResponse.json(await evaluationRunService.getRun(context.user.$id,id,runId))}catch(error){if(error instanceof EvaluationError)return NextResponse.json({error:error.message,code:error.code},{status:error.status});console.error("[EvaluationRunDetail] failed",error);return NextResponse.json({error:"Failed to read evaluation run"},{status:500})}}
