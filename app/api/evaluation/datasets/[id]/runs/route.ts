...
async function handler(
  ...
  try {
    ...
    if (request.method === "GET") {
      const runs = await evaluationRunService.listRuns(
        context.user.$id,
        id,
        {
          limit: page(request.nextUrl.searchParams, "limit", 20),
          offset: page(request.nextUrl.searchParams, "offset", 0),
        },
      )
    ...
    console.error("[EvaluationRuns] failed", error)

    return NextResponse.json(
      { error: "Evaluation run operation failed" },
      { status: 500 },
    )
  }
}
