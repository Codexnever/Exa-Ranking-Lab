...
async function handler(
  ...
  try {
    ...
    const queryIds = parseQueryIds(
      await request.json().catch(() => {
        throw invalid("Request body must be valid JSON")
      }),
    )

    const dataset = await evaluationDatasetService.addOperationalQueries(
      context.user.$id,
      id,
      queryIds,
    )
    ...
    console.error("[EvaluationDatasetQueries] failed", error)

    return NextResponse.json(
      { error: "Failed to add benchmark queries" },
      { status: 500 },
    )
  }
}
