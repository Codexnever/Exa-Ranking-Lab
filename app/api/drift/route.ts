...
export async function GET(request: NextRequest) {
  try {
    ...
    const snapshots = await databaseService.snapshotService.getSnapshots(user.$id)
console.log('Drift analysis started for user:', user.$id, queries.length, 'queries and', snapshots.length, 'snapshots')
    // Analyze drift for all queries
    const driftResults = analyzeDriftForQueries(
      queries.map((q) => ({ id: q.id, name: q.name })),
      snapshots,
    )

    // Sort by latest drift score (highest first)
